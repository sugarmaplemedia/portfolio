...
export default (props: galleryProps) => {
	...
	props.tag == ""
		...
		: (galleryEntries = props.database.entries.filter((entry) =>
				entry.tags.includes(props.tag),
			))

	let count
	props.count == 0 || props.count > galleryEntries.length
	...
	for (let i = 0; i < count; i++) {
		galleryArr.push(
			<a
				key={galleryEntries[i].name}
				className={clsx(styles["item"], {
					[styles["small"]]: props.size === "sml",
				})}
				href={galleryEntries[i].link}
			>
				<div
					className={styles["image-box"]}
					style={{
						backgroundImage: `url(${galleryEntries[i].image})`,
					}}
				></div>
				<div className={styles["text-box"]}>
					<h4 className={styles["title"]}>
						{galleryEntries[i].name}
					</h4>
